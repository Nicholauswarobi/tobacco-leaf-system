...
export default function HistoryPage() {
  ...
  return (
    <section className="mx-auto max-w-7xl px-3 sm:px-3 py-5 sm:py-6">
      <header className="flex flex-wrap items-end justify-between gap-2.5 mb-3">
        <div>
          ...
          <p className="mt-2 text-[var(--fg-muted)] max-w-xl">{t("hist.lead")}</p>
        </div>
      ...
      <div className="mb-3 flex flex-col sm:flex-row gap-2">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full h-11 pl-11 pr-4 rounded-sm border border-[var(--border)] bg-[var(--bg-elev)] text-sm placeholder:text-[var(--fg-muted)] focus:outline-none focus:border-leaf-600"
          />
        </div>
      ...
      {notice && (
        <div className="mb-3 rounded border border-red-200 bg-red-50 px-3 py-2 text-sm text-red-800 dark:border-red-900/40 dark:bg-red-950/30 dark:text-red-200">
          {notice}
        </div>
      )}
        ...
        <div className="rounded border border-[var(--border)] bg-[var(--bg-elev)] overflow-hidden">
          {loading ? (
            ...
            <div className="p-16 text-center">
              <Leaf className="h-8 w-8 mx-auto mb-2 text-[var(--fg-muted)] opacity-50" />
              ...
              {items.length === 0 && (
                <Link
                  href="/disease"
                  className="mt-2 inline-block text-leaf-700 dark:text-leaf-300 hover:underline"
                >
                  {t("hist.emptyCta")} →
                </Link>
              )}
            ...
            <>
              ...
              <table className="hidden md:table w-full text-sm">
                <thead className="bg-leaf-50 dark:bg-leaf-900/20 text-xs font-semibold tracking-wide text-[var(--fg-muted)]">
                  <tr>
                    <th className="text-left px-3 py-2 font-medium">{t("hist.colLeaf")}</th>
                    <th className="text-left px-3 py-2 font-medium">{t("hist.colType")}</th>
                    ...
                    <th className="text-left px-3 py-2 font-medium">{t("hist.colWhen")}</th>
                    <th className="text-right px-3 py-2 font-medium">{t("hist.colActions")}</th>
                  </tr>
                </thead>
                      ...
                      <tr key={it.id} className="hover:bg-leaf-50/40 dark:hover:bg-leaf-900/10">
                        <td className="px-3 py-2">
                          {it.image_url ? (
                            ...
                            <div className="h-10 w-10 rounded-sm bg-leaf-100 dark:bg-leaf-800/30 flex items-center justify-center">
                        ...
                        <td className="px-3 py-2 text-right">
                          <button
                            ...
                            className="inline-flex h-8 w-8 items-center justify-center rounded-sm text-[var(--fg-muted)] hover:bg-red-50 hover:text-red-700 dark:hover:bg-red-900/20"
                          >
              ...
              <div className="md:hidden divide-y divide-[var(--border)]">
                {filtered.map((it) => {
                  ...
                  const isQualityMode = it.mode === "quality";
                  return (
                    <div key={it.id} className="p-2 flex items-start gap-2">
                      {it.image_url ? (
                        /* eslint-disable-next-line @next/next/no-img-element */
                        <img
                          src={api.asset(it.image_url)}
                        ...
                        <div className="h-14 w-14 rounded bg-leaf-100 dark:bg-leaf-800/30 flex items-center justify-center shrink-0">
                      ...
                      <button
                        onClick={() => remove(it.id)}
                        aria-label={t("hist.colActions")}
                        className="inline-flex h-9 w-9 items-center justify-center rounded-sm text-[var(--fg-muted)] hover:bg-red-50 hover:text-red-700"
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
